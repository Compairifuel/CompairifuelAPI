...
public class AuthorizationController {
    ...
    public Response getAuthorizationCode(@QueryParam("response_type") @Pattern(regexp = "(code)", message = "response_type must be set to “code”.") String responseType, @QueryParam("client_id") @NotBlank(message = "client_id cannot be blank!") String clientId, @QueryParam("client_secret") String clientSecret, @QueryParam("redirect_uri") @NotBlank(message = "redirect_uri cannot be blank!") @RedirectURI(message = "redirect_uri must be a valid uri.") String redirectUri, @QueryParam("code_challenge") @NotBlank(message = "code_challenge cannot be blank!") String codeChallenge, @QueryParam("state") @NotBlank(message = "state cannot be blank!") String state, @QueryParam("scope") @Pattern(regexp = "^([A-Za-z: ]*)$", message = "scope must be a valid format.") String scope) {
        scope = (scope == null) ? "" : scope.replaceAll("\\s+", " ").trim();
        if (!scope.isEmpty()) {
            for (String s : scope.split(" ")) {
                String tokenRegex = "[A-Za-z]+(:[A-Za-z]+){0,2}";
                if (!s.matches(tokenRegex)) {
                    log.warning("scope is not in a valid format!");
                    throw new BadRequestException("scope is not in a valid format!");
                }
                
            }
        }
        
        URI redirectToURI = authorizationService.getAuthorizationCode(responseType, clientId, clientSecret, redirectUri, codeChallenge, state, scope);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response getAccessToken(@FormParam("grant_type") @Pattern(regexp = "authorization_code", message = "grant_type must be set to “authorization_code”.") String grantType, @FormParam("code") @NotBlank(message = "code cannot be blank!") String code, @FormParam("redirect_uri") @NotBlank(message = "redirect_uri cannot be blank!") @RedirectURI(message = "redirect_uri must be a valid uri.") String redirectUri, @FormParam("client_id") @NotBlank(message = "client_id cannot be blank!") String clientId, @FormParam("client_secret") String clientSecret, @FormParam("code_verifier") @NotBlank(message = "code_verifier cannot be blank!") String codeVerifier) {
        AccessTokenDomain accessTokenDomain = authorizationService.getAccessToken(grantType, code, redirectUri, clientId, clientSecret, codeVerifier);
    ...
    @Path("/refresh")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getAccessTokenByRefreshToken(@QueryParam("grant_type") @Pattern(regexp = "(refresh_token)", message = "grant_type must be set to “refresh_token”.") String grantType, @QueryParam("refresh_token") @NotBlank(message = "refresh_token cannot be blank!") String refreshToken, @QueryParam("client_id")  @NotBlank(message = "client_id cannot be blank!") String clientId, @QueryParam("client_secret") String clientSecret, @QueryParam("code_verifier") @NotBlank(message = "code_verifier cannot be blank!") String codeVerifier) {
        AccessTokenDomain accessTokenDomain = authorizationService.getAccessTokenByRefreshToken(grantType, refreshToken, clientId, clientSecret, codeVerifier);
